...
As soon as we set my_int equal to the value of [i]103204934813*, we can use *my_int[/i] in the place of the integer, so let’s print it out:
...
In this example, Python does the math for us, subtracting [i]813[/i] from the variable [i]my_int[/i] to return the sum [i]103204934000.[/i] Speaking of math, variables can be set equal to the result of a math equation. Let’s add two numbers together and store the value of the sum into the variable *x*:
[code]x = 76 + 145[/code]
...
Something else to keep in mind when naming variables, is that they are case-sensitive, meaning that [i]my_int*, *MY_INT*, *My_Int*, and *mY_iNt[/i] are all completely different variables.
